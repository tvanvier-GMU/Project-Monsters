...
public class Battle : MonoBehaviour {
	...
	public void UseBattleMove(int index, Monster targetMonster){
		BattleMove move = playerMonster.moves [index];

		if (move != null && targetMonster != null) {
			//Roll to see if the move HITS
			int hitRoll = Random.Range (0, 100);
			if (!(hitRoll <= move.baseAccuracy)) {
				//Miss!
				Debug.Log(move.attackName + " missed!");
				return;
			}

			if (playersTurn) {
				opponentMonster.currentHealth -= move.baseDamage;
				if (move.statusEffects != null && move.statusEffects.Length > 0) {
					foreach (StatusEffect affliction in move.statusEffects) {
						int statusRoll = Random.Range (0, 100);
						if (statusRoll <= affliction.chance) {
							Debug.Log ("status inflicted");
							opponentMonster.statusAffliction = affliction.status;
							break; //stop on the first status hit
						}
					}
				}
			}
